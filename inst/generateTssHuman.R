...
tss <- getTssObjectFromTxObject(txdb_human)
...
appris <- read.table("appris/appris_data.principal_human.txt.gz",
                     sep="\t", head=TRUE)
...
wh <- match(gr$gene_id, txdb_human$cds$gene_id)
gr$gene_symbol <- txdb_human$cds$gene_symbol[wh]
names(gr) <- gr$ID
tss_human <- gr
